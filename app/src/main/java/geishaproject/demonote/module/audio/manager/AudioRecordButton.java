package geishaproject.demonote.module.audio.manager;
...
public class AudioRecordButton extends android.support.v7.widget.AppCompatButton implements AudioManager.AudioStageListener{
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("JabinGP","开始switch");
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                Log.d("JabinGP","ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE:
                Log.d("JabinGP","ACTION_MOVE");
                if (isRecording) {
            ...
            case MotionEvent.ACTION_UP:
                Log.d("JabinGP","ACTION_UP");
                // 首先判断是否有触发onlongclick事件，没有的话直接返回reset
                if (!mReady) {
                    Log.d("JabinGP","if(!mReady)");
                    reset();
                    return super.onTouchEvent(event);
                }
                // 如果按的时间太短，还没准备好或者时间录制太短，就离开了，则显示这个dialog
                Log.d("JabinGP","GoingToRecording");
                Log.d("JabinGP","get !isRecording || mTime < 0.8f: -- "+(!isRecording || mTime < 0.8f));
                if (!isRecording || mTime < 0.8f) {
                    Log.d("JabinGP","if(!isRecording ||  mTime < 0.8f)");
                    mDialogManager.tooShort();
                    mAudioManager.cancel();
                    mStateHandler.sendEmptyMessageDelayed(MSG_DIALOG_DIMISS, 1300);// 持续1.3s
                } else if (mCurrentState == STATE_RECORDING) {//正常录制结束

                    Log.d("JabinGP","正常录制结束");
            ...
            default:
                //解决小米系统上无法触发ActionUp的错误
                Log.d("defaultJabinGP","ACTION_UP");
                // 首先判断是否有触发onlongclick事件，没有的话直接返回reset
                if (!mReady) {
                    Log.d("JabinGP","if(!mReady)");
                    reset();
                ...
                // 如果按的时间太短，还没准备好或者时间录制太短，就离开了，则显示这个dialog
                Log.d("defaultJabinGP","GoingToRecording");
                Log.d("defaultJabinGP","get !isRecording || mTime < 0.8f: -- "+(!isRecording || mTime < 0.8f));
                if (!isRecording || mTime < 0.8f) {
                    Log.d("defaultJabinGP","if(!isRecording ||  mTime < 0.8f)");
                    mDialogManager.tooShort();
                    mAudioManager.cancel();
                    mStateHandler.sendEmptyMessageDelayed(MSG_DIALOG_DIMISS, 1300);// 持续1.3s
                } else if (mCurrentState == STATE_RECORDING) {//正常录制结束

                    Log.d("defaultJabinGP","正常录制结束");
                    if (isOverTime) return super.onTouchEvent(event);//超时
                    mDialogManager.dimissDialog();
                    mAudioManager.release();// release释放一个mediarecorder
